...
$(document).ready(function() {
	...
	$('header .menu li.clickable').on('click', function(event) {
		...
		if (active) {
			...
			const activeSection = $('header .menu li.clickable.active').data('section');
			$('header .menu li').removeClass('active');
	...
	$('#mobileVideo').on("click", function(e) {
		e.preventDefault();
	...
	$('#cv').on('change', function(e) {
		...
		if (files.length > 0) {
			...
			$('.attachFile').find('span').text(`(${filename})`);
		} else {
			$('.attachFile').find('span').empty();
		};
	});
