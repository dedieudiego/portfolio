...
class ThefText 
{
	 ...
	 * @param string $email 
	 * @return Boolean True for valid email
	 */
	 ...
	 */
	public static function cut($text = '', $length = 150) 
	{
		$result = '';
    ...
    public static function twitter_users($text) 
    {
        $text = preg_replace('/@(\w+)/', "<a href=\"http://twitter.com/#!/$1\" class=\"twitter-user\" target=\"_blank\">$0</a>", $text);
        return $text;
    }
	...
	public static function cleanHtmlCustom($text, $length = 0) 
	{
		$text = preg_replace('/(SIZE=")([0-9]{1,2})(")/', '', stripslashes($text));
		$text = str_ireplace('LETTERSPACING="0"', '', $text);
	...
	public static function inputPrint($texto) 
	{
	 ...
	 * @param Integer $largo Length for the token (Default: 50)
	...
	public static function generarToken($largo = 50) 
	{
		...
		$cadena = $time.'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
	...
	{
		$text = str_replace('"', '\"', filter_var($text, FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES));
		return $text;
	}
    ...
    public static function textOptimize($text)
    {  
        $text = str_replace('&amp;#39;', '\'', htmlentities($text, ENT_NOQUOTES, 'UTF-8'));
        $text = str_replace('&amp;#34;', '"', $text);
        return $text;
    }	

    public static function optimizeTweeterFeed($str)
    {
		return ThefText::twitter_users(ThefText::hyperlinks(ThefText::textOptimize($str)));
    }
