...
{
  public $tpl_base;              //Array( [filename/varcontent], [T_BYFILE/T_BYVAR] )
  public $tpl_include;           //Array( [filename/varcontent], [T_BYFILE/T_BYVAR] )
  public $tpl_count;
  ...
  public $seoKeywords = '';
   ...
   function __prepare()
   {
    ...
    function __prepareTemplate( $tpl_file, $type )
    {
        ...
        {
        	if (!isset($this->original_html) || !$this->original_html) {
        		//pr("voy a leer <BR>");
        		$this->original_html = @file( $tpl_file ) or
                die( $this->__errorAlert('TemplatePower Error: Couldn\'t open [ '. $tpl_file .' ]!'));
        	} else {
        		//pr("lo tenia en cache <BR>");
        	}
			$this->{$tplvar}["content"] = $this->original_html;
        }
...
class TemplatePower extends TemplatePowerParser
{
  ...
  /**********
	...
	/***************************** SEO  **********************************************************/
	// $keys: string de valores separados por comas.
	public function seoKeywordsAdd($keys = '')
	{
		$keys = trim($keys);
		if ($this->seoKeywords != '') {
			$keys = ', ' . $keys;
		}
		$this->seoKeywords .= $keys;
	}
	public function fillSeo($url = '')
	{
		if ($url == '') {
			//$url = $_SERVER['REQUEST_URI'];
			$url = $_SERVER['SCRIPT_URI'];
		}
		$persist = new PersistenceMysql();
		$seo = $persist->mysqlObjResult("SELECT * FROM seo_url WHERE url = '$url'");
		if (!$seo) {
			$seo = $persist->mysqlObjResult('SELECT * FROM seo_default'); // Valores por defecto
		}
		$this->seoKeywordsAdd($seo->page_keywords);
		$this->gotoBlock('_ROOT');
		$this->assign('PAGE_TITLE', $seo->page_title);
		$this->assign('PAGE_DESCRIPTION', $seo->page_description);
		$this->assign('PAGE_KEYWORDS', $this->seoKeywords);
		$this->assign('PAGE_LANGUAGE', $seo->idioma);
	}
	/***************************** END SEO  **********************************************************/
}
